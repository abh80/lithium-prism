...
export const readCssNumber = (styles: CSSStyleDeclaration, name: string) => {
...
export const readCssString = (styles: CSSStyleDeclaration, name: string) => {
...
const getStripeCount = (canvasWidth: number) => {
    ...
    const targetCount = Math.max(1, Math.round(canvasWidth / STRIPE_WIDTH.target));

    return Math.max(minCount, Math.min(maxCount, targetCount));
};

const getRevealDelay = (index: number, stripeCount: number, stagger = INTRO.stagger) => {
...
export const getIdleCenter = (stripe: Stripe, wavePhase: number, secondaryWavePhase: number) => {
...
const getSpeedUpRampUpDuration = (from: WaveSpeedUpValues) => {
    const multiplierRemaining = clamp(
        (WAVE_SPEED_UP.multiplier - from.multiplier) / (WAVE_SPEED_UP.multiplier - 1),
    );
    const shineRemaining = 1 - from.shineProgress;

    return Math.max(
        1,
        WAVE_SPEED_UP.rampUpDuration * Math.max(multiplierRemaining, shineRemaining),
    );
};
...
export const triggerSpeedUpAnimation = (
    ...
    const { multiplier, shineProgress } = updateSpeedUpAnimation(time, speedUpAnimation);
...
export const updateSpeedUpAnimation = (
    ...
    if (speedUpAnimation.phase === "ramp-up") {
        const progress = getProgress(time, speedUpAnimation.startedAt, speedUpAnimation.duration);
    ...
    if (speedUpAnimation.phase === "hold") {
        const progress = getProgress(time, speedUpAnimation.startedAt, WAVE_SPEED_UP.waveDuration);
    ...
    const progress = getProgress(time, speedUpAnimation.startedAt, WAVE_SPEED_UP.rampDownDuration);
    const values = lerpSpeedUpValues(
        speedUpAnimation.from,
        IDLE_SPEED_UP,
